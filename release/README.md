...
The current successor is ReleaseManifest/EvidenceIndex v9. It preserves the immutable v1-v8 profiles and every v8 input, then additively binds the quality-finalization lock, PIC 1.2 same-task close contract, strict observation/finalization API, and ingestion-quality runtime/role v6 successors. Story 2.5c adds a positive executable closure for durable real-fact observation, fresh final D4, the ingestion-quality owner-local `recovering -> eligible` transaction, same-task close, delivery compensation, and new-generation relapse. Story 3.2 consumption, Story 5.5 final public apply, Story 2.7c NFR8 final evidence, and production-duration evidence remain explicitly deferred with `runtimeEvidenceClaim: none`; local or CI evidence cannot be relabelled as those future production claims.
