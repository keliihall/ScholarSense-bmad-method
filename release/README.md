...
The current successor is ReleaseManifest/EvidenceIndex v4. It preserves the immutable v1-v3 profiles, adds the exact SubjectRegistry contract lock and deployment runtime profile as controlled inputs, and records production KMS as `deployment-input-required` with `runtimeEvidenceClaim: none`. Local or CI contract evidence must never be promoted into a false production-KMS claim.
