...
The current successor is ReleaseManifest/EvidenceIndex v5. It preserves the immutable v1-v4 profiles, adds the exact ingestion batch quality, snapshot hash, event, runtime and role successor inputs, and records quality-worker target activation as `deployment-input-required` with `runtimeEvidenceClaim: none`. SubjectRegistry production KMS retains the same honest boundary. Local or CI contract evidence must never be promoted into a false production-runtime claim.
