...
class ReleaseWorkflowContractTest(unittest.TestCase):
    def test_release_v3_runs_protected_target_jobs_and_verifies_their_immutable_graph(self) -> None:
        ...
        self.assertIn("--manifest-version 5", release)
        self.assertIn(
            "application/vnd.scholarsense.release-manifest.v5+json", release
        )
        self.assertIn(
            "application/vnd.scholarsense.evidence-index.v5+json", release
        )
        ...
        self.assertIn("RELEASE-MANIFEST-5.0.0", verifier)
