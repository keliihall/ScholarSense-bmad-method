...
public final class TrustedTimeSubjectRegistryIds implements SubjectRegistryIdPort {
    private final TrustedTimeSource time;
    private final SecureRandom random = new SecureRandom();

    public TrustedTimeSubjectRegistryIds(TrustedTimeSource time) {
        this.time = java.util.Objects.requireNonNull(time);
    }

    @Override
    public UUID nextUuid() {
        Instant instant = time.now().instant();
